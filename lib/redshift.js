...
Redshift.prototype.latestFullMonth = function () {
  let query = `SELECT MAX(statement_month) FROM ${this.lineItemsTableName};`
...
Redshift.prototype.importFullMonth = function (s3uri, month, pruneThresholdMonths) {
  ...
  if (typeof pruneThresholdMonths === 'number') {
    ...
    let pruneThresholdString = moment(month)
      ...
      .format('YYYY-MM-01')
    pruneQueryFragment = `DELETE FROM line_items WHERE statement_month < '${pruneThresholdString}'::DATE;`
  }
  ...
  let query = `
    BEGIN;
      ...
      DELETE FROM ${this.lineItemsTableName} WHERE statement_month = '${monthDateString}';
      INSERT INTO ${this.lineItemsTableName} SELECT * FROM ${stagingTableName};
...
Redshift.prototype.importMonthToDate = function (s3uri) {
  let self = this
  let truncateQuery = `TRUNCATE month_to_date;`
